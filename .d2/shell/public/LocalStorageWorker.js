/* Return code: 0 succes, 1 erro */
onmessage = function(message) {
    try {
        if (message.data[0] === 'compress') {
            const data = message.data[1]
            console.log(
                'Size before compressing in mb (ca): ',
                data.length / 1000000
            )
            const compressed = compressToUTF16(data)
            console.log(
                'Size after compressing in mb (ca):  ',
                compressed.length / 1000000
            )
            postMessage([true, compressed])
        } else if (message.data[0] === 'decompress') {
            const data = message.data[1]
            console.log(
                'Size before decompressing in mb (ca): ',
                data.length / 1000000
            )
            const decompressed = decompressFromUTF16(data)
            console.log(
                'Size after decompressing in mb (ca):  ',
                decompressed.length / 1000000
            )
            postMessage([true, decompressed])
        } else {
            throw Error('UNKNOWN COMMAND!')
        }
    } catch (err) {
        console.error('Error at worker thread')
        postMessage([false, err])
    }
}
